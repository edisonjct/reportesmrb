...
$pdf->SetFont('Arial', '', 8);
//CONSULTA
$productos = mysql_query("SELECT id_prod,nomb_prod FROM productos WHERE precio_unit = '$dato' ");
$item = 0;
$totaluni = 0;
$totaldis = 0;
while($productos2 = mysql_fetch_array($productos)){
	$item = $item+1;
	$totaluni = $totaluni + $productos2['id_prod'];
	$totaldis = $totaldis + $productos2['nomb_prod'];
	$pdf->Cell(15, 8, $item, 0);
	$pdf->Cell(70, 8,$productos2['id_prod'], 0);
	$pdf->Cell(40, 8, $productos2['nomb_prod'], 0);
	$pdf->Cell(25, 8, 'S/. '.$productos2['id_prod'], 0);
	$pdf->Cell(25, 8, 'S/. '.$productos2['nomb_prod'], 0);	
	$pdf->Ln(8);
}
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(104,8,'',0);
